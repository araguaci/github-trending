...
";
    ...
    while ($file = readdir($handle)) {
        ...
        //echo chr(13) . "file: " . $file . chr(13);
        if ( (substr($file, -3)==".md") && ($file!="index.md") ) {
            $mescorrent = substr($file, 5, 2);
            //echo chr(13) . "mescorrent: " . $mescorrent . chr(13);
            $strdate = str_replace(".md", "", $file);
            if ($mescorrent != $mes) {
                //echo chr(13) . "strdate: " . $strdate . chr(13);
    //$date = new DateTime($strdate);
    //echo $date->format('F'); // June            
                $mes = $mescorrent;
                $strMarkdown .= "

### " . $monthNames[intval($mescorrent)] . " - " . $mes . "
    ";
            }
            if (is_file($file) && strstr($file, '.md')) {
                $cnt++;
                $msg = '';
                $strMarkdown .= "
- [" . str_replace(".md", "", $file) . "](https://araguaci.github.io/github-trending/#/2025/" . $strdate . ")";
            } 
        }
    }
    closedir($handle);	
	echo $strMarkdown;
    $myfile = fopen("./index.md", "w");
    fwrite($myfile, $strMarkdown);
    fclose($myfile);
?>
<?php
